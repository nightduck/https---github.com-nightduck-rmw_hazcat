...
void cpu_copy(struct hma_allocator * dest_alloc, void * dest, void * src, size_t size);
...
#define ALLOCATE(alloc, size) \
  (allocate_fps[alloc->strategy * NUM_DEV_TYPES + alloc->device_tpye])(alloc, size)

// Deallocates memory at offset previously generated by alloc
#define DEALLOCATE(alloc, offset) \
  (deallocate_fps[alloc->strategy * NUM_DEV_TYPES + alloc->device_tpye])(alloc, offset)
...
#define COPY_TO(alloc, alloc_mem, cpu_mem, size) \
  (copy_to_fps[alloc->strategy * NUM_DEV_TYPES + alloc->device_tpye])(alloc_mem, cpu_mem, size)

// Copies alloc_mem in alloc to main memory pointer at cpu_mem, of size size
#define COPY_FROM(alloc, alloc_mem, cpu_mem, size) \
  (copy_from_fps[alloc->strategy * NUM_DEV_TYPES + alloc->device_tpye])(alloc_mem, cpu_mem, size)
...
#define COPY(dest_alloc, dest_mem, src_alloc, src_mem, size) \
  (copy_fps[src_alloc->strategy * NUM_DEV_TYPES + src_alloc->device_tpye])(
